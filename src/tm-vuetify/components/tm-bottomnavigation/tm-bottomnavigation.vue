	...
	<view :style="{
		...
		<view :style="{ background: bgColor }" class=" flex-between ">
			...
			<block v-for="(item, index) in listDate" :key="index">
				<view :style="{
						width: listLen + '%'
					}" class="flex-center" :key="index" :class="[$tm.vx.state().tmVuetify.tmVueTifly_pagesIndex == index && ani == true ? ` ani ` : ``]">
					
						<tm-button :key="index" titl height="100%" :width="btnwidth"
							:iconSize="item['iconSize'] ? item['iconSize'] : 38"
							:fontSize="item['fontSize'] ? item['fontSize'] : 20" :vertical="vertical" :icon="$tm.vx.state().tmVuetify.tmVueTifly_pagesIndex==index?item.icon:(item['noIcon']||item.icon)"
							:label="item.value" block :theme="$tm.vx.state().tmVuetify.tmVueTifly_pagesIndex == index ? iconColor : iconColorGrey"
							:black="black_tmeme" @click.stop="onclick($event, index)" item-class="noGutter  "
							:font-color="$tm.vx.state().tmVuetify.tmVueTifly_pagesIndex == index ? iconColor : iconColorGrey">
								...
								<view style="height: 52rpx;">
									<view v-if="!item['custom']"  >
										...
										<tm-icons :black="black_tmeme"
											:color="$tm.vx.state().tmVuetify.tmVueTifly_pagesIndex == index ? iconColor : iconColorGrey" :size="data.size"
							...
							<template v-slot:default="{data}">
								<text :class="[$tm.vx.state().tmVuetify.tmVueTifly_pagesIndex == index ? 'ani' : '']">{{ data }}</text>
							</template>
						</tm-button>
		...
		methods: {
			autoxz(){
				...
				for(let i=-1;i<this.listDate.length;i++){
					if(this.listDate[i]?.path == pageRoute){
