<template>
	<view class="tm-quickIndex " :style="{
		...
		<view :style="{
			...
			<scroll-view scroll-y :class="[black_tmeme?'grey-darken-4':'common-bg']" :style="{
					height: activeHeight_watch + 'px',
				...
				<view v-for="(item,index) in dataList" :key="index" :id="guid+'_'+index" class="tm-quickIndex-item">
					...
					<view :class="[black_tmeme?'grey-darken-5':'grey-lighten-4 text']"
						class="  text-size-sn text-weight-b px-32 py-12">{{item.title}}</view>
					<view :style="cellStyle">

						<view v-for="(item2,index2) in item.children" :key="index2">

							<slot name="cell"
								:data="{prevent:index,children:index2,total:item.children.length,item:item2,title:item2[rangKey],color:color_tmeme,black:black_tmeme}">
								<view
									:class="[index2!==item.children.length-1?'border-grey-lighten-4-b-1 ':'',black_tmeme?'bk':'']"
									class="flex-start" @click="changeIndex(index,index2,item2)">
									<view v-if="item2['icon']" style="width: 48rpx;height: 48rpx;"
										class="mr-24 rounded flex-center overflow">
										<tm-icons :size="48" :name="item2['icon']"></tm-icons>
									</view>
									<view class="subjectBox text-size-sn" 
									:class="(index===parentNum&&index2===childrenNum)?[$tm.vx.state().tmVuetify.color]:''">
										{{item2[rangKey]}}
									</view>
								</view>
	...
	<!-- 	<view class="tm-quickIndex-index flex-center flex-col pr-16" :style="{
			...
			<view v-if="showtips" :class="[`text-${color_tmeme}`,black_tmeme?'bk':'']"
			...
			<view v-if="activeHeight_watch>0" @touchend.stop.prevent="indexMove($event,'end')"
				@touchmove.stop.prevent="indexMove($event,'scroll')" class="tm-quickIndex-index-Bk  round-24  shadow-3 "
				:class="[black_tmeme?'grey-darken-5 bk':'white']">
	...
	export default {
		...
		watch: {
			...
			list: {
				deep: true,
				handler() {
					this.indexList = this.getDataList()
		...
		methods: {
			...
			changeIndex(prentindex, childrenindex, item) {
				this.parentNum=prentindex
				this.childrenNum = childrenindex
				this.$emit('change', {
					prent: prentindex,
