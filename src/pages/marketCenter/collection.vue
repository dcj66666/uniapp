		...
		<tm-pullBottom :height="1490" :loading.sync="loading" @refresh="getdata">
			...
			<template v-slot:default>
				<view v-if="!ifNoData">
					<view class=" tab-box">
						<view class="tab-list">
							<view v-for="(item,index) in favoriteList" :key="index" class="tab-style"
								@tap="activeNum=index" :class="activeNum==index?'text-main text-weight-b':''">
								<view v-show="activeNum==index" class="acitiveNum"></view>
								<text class="tab-text">{{item.projectName}}</text>
							</view>
						</view>
					</view>
					<view v-for="(item,idx) in favoriteList[activeNum].favSubQueList" :key="idx">
							...
							<view class="flex flex-between">
								...
								<view class="exercise-button  flex-center mb--50 common-click">
									...
									<text class="ml-6 text-size-sl"
										@tap="review(item.questionIds,item.subjectId,favoriteList[activeNum].projectId,item.subjectName,favoriteList[activeNum].projectName)">开始复习</text>
								</view>
							</view>
							<view class="mt-6">
								<text class="text-size-n text-main">{{item.questionCount}}</text>
								<text class="text-size-sl">题</text>
							</view>
						</view>
					</view>
				</view>
				<Empty v-if="ifNoData" text="暂无收藏试题" emptyImg="../../static/img/empty/image-105.png">
				</Empty>
			</template>
		</tm-pullBottom>
	</view>
...
<script lang="ts">
	...
	export default class wrongList extends Vue {
		...
		review(questionIds: number[], subjectId: number, projectId: number, subjectName: string, projectName: string) {
			let url =
				`/pages/marketCenter/brushQuestions?questionIds=${questionIds}&subjectId=${subjectId}&projectId=${projectId}&subjectName=${subjectName}&projectName=${projectName}&isFavorites=true`;
			uni.navigateTo({
				url
			});
		}
